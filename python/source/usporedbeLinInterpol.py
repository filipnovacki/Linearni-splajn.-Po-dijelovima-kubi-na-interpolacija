...
def crtaj_usporedbe_kvadr():
    ...
    while i <= 40:
        ...
        # Racunanje linearne interpolacije
        opseg = np.arange(-5.00001 + i * const, -5 + (i + 1) * const, const)
        vrijednosti = 1 / (np.square(opseg) + 1)
        plt.plot(opseg, vrijednosti)

        # Racunanje kvaziprave funkcije
        opseg_detaljan = np.arange(-5.00001 + i * const, -5.00001 + (i + 1) * const, 0.0001)
